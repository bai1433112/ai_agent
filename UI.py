...
# 核心逻辑：根据模式处理对话
if input := st.chat_input("来和我聊天吧~~~", accept_file="multiple", file_type=['png', 'jpg', 'jpeg', "xlsx", "csv"]):
    if input.text:  # 检查是否有文本内容
        ...
        if not st.session_state.is_voice_mode:
            try:
                ...
                response = requests.post(
                    CHAT_ENDPOINT,
                    json={"prompt": input.text},  # 使用JSON格式传递
                    params={"mode": "nlp"}
                )
                print(response.json())
                ...
                # 更新对话历史并显示
                st.session_state.history.append({"role": "human", "content": input.text, "type": "text"})
                st.session_state.history.append({"role": "assistant", "content": response.json(), "type": "text"})
