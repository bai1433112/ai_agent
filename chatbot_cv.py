...
class Cv_Chatbot(ChatBot):
    ...
    def get_prompt(self, base64_image, text="描述图片"):
        # 1. 创建“多模态内容列表”：里面装两个东西——文字指令 + 图片（Base64格式）
        multimodal_content = [
            # 第一个元素：文字指令（告诉模型要做什么）
            {"type": "text", "text": text},
            # 第二个元素：图片内容（让模型“看见”的东西）
            {
                "type": "image_url",  # 告诉模型：这是图片类型的内容
                "image_url": {
                    # 关键：图片的 Base64 字符串，按“data:image/jpeg;base64,xxx”格式拼接
                    # 模型看到这个格式，就知道要把后面的字符串解码成图片
                    "url": f"data:image/jpeg;base64,{base64_image}"
                }
            }
        ]
        # 2. 返回这个“打包好的指令包”，直接传给模型
